...
# Verifies that skills are resolved with correct priority: project > personal > omnipowers
...
# 1. Create in omnipowers location (lowest priority)
mkdir -p "$HOME/.config/opencode/omnipowers/skills/priority-test"
cat > "$HOME/.config/opencode/omnipowers/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Omnipowers version of priority test skill
---
# Priority Test Skill (Omnipowers Version)

This is the OMNIPOWERS version of the priority test skill.

PRIORITY_MARKER_OMNIPOWERS_VERSION
EOF
...
if [ -f "$HOME/.config/opencode/omnipowers/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Omnipowers version exists"
else
    echo "  [FAIL] Omnipowers version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides omnipowers
echo ""
echo "Test 2: Testing personal > omnipowers priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides omnipowers)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_OMNIPOWERS_VERSION"; then
    echo "  [FAIL] Omnipowers version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|omnipowers" | head -10
fi

# Test 3: Test that project overrides both personal and omnipowers
echo ""
echo "Test 3: Testing project > personal > omnipowers priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Omnipowers version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit omnipowers: prefix bypasses priority
echo ""
echo "Test 4: Testing omnipowers: prefix forces omnipowers version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load omnipowers:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_OMNIPOWERS_VERSION"; then
    echo "  [PASS] omnipowers: prefix correctly forces omnipowers version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] omnipowers: prefix did not force omnipowers version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
