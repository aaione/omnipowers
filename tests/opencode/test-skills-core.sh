...
# Create skills in personal and omnipowers locations for testing
mkdir -p "$TEST_HOME/personal-skills/shared-skill"
mkdir -p "$TEST_HOME/omnipowers-skills/shared-skill"
mkdir -p "$TEST_HOME/omnipowers-skills/unique-skill"
...
cat > "$TEST_HOME/omnipowers-skills/shared-skill/SKILL.md" <<'EOF'
---
name: shared-skill
description: Omnipowers version
---
# Omnipowers Shared
EOF

cat > "$TEST_HOME/omnipowers-skills/unique-skill/SKILL.md" <<'EOF'
---
name: unique-skill
description: Only in omnipowers
---
# Unique
EOF
...
const omnipowersDir = '$TEST_HOME/omnipowers-skills';
const personalDir = '$TEST_HOME/personal-skills';

// Test 1: Shared skill should resolve to personal
const shared = resolveSkillPath('shared-skill', omnipowersDir, personalDir);
console.log('SHARED:', JSON.stringify(shared));

// Test 2: omnipowers: prefix should force omnipowers
const forced = resolveSkillPath('omnipowers:shared-skill', omnipowersDir, personalDir);
console.log('FORCED:', JSON.stringify(forced));

// Test 3: Unique skill should resolve to omnipowers
const unique = resolveSkillPath('unique-skill', omnipowersDir, personalDir);
console.log('UNIQUE:', JSON.stringify(unique));

// Test 4: Non-existent skill
const notfound = resolveSkillPath('not-a-skill', omnipowersDir, personalDir);
console.log('NOTFOUND:', JSON.stringify(notfound));
" 2>&1)

if echo "$result" | grep -q 'SHARED:.*"sourceType":"personal"'; then
    echo "  [PASS] Personal skills shadow omnipowers skills"
else
...
if echo "$result" | grep -q 'FORCED:.*"sourceType":"omnipowers"'; then
    echo "  [PASS] omnipowers: prefix forces omnipowers resolution"
else
    echo "  [FAIL] omnipowers: prefix not working"
    exit 1
fi

if echo "$result" | grep -q 'UNIQUE:.*"sourceType":"omnipowers"'; then
    echo "  [PASS] Unique omnipowers skills are found"
else
    echo "  [FAIL] Unique omnipowers skills not found"
    exit 1
fi
