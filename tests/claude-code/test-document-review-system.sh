...
mkdir -p docs/omnipowers/specs

# Create a spec document WITH INTENTIONAL ERRORS for the reviewer to catch
cat > docs/omnipowers/specs/test-feature-design.md <<'EOF'
# Test Feature Design
...
Then review the spec at $TEST_PROJECT/docs/omnipowers/specs/test-feature-design.md using the criteria from that template.
...
if grep -qi "Issues Found\|\xe2\x9d\x8c\|not approved\|issues found" "$OUTPUT_FILE"; then
    echo "  [PASS] Reviewer correctly found issues (not approved)"
elif grep -qi "Approved\|\xe2\x9c\x85" "$OUTPUT_FILE" && ! grep -qi "Issues Found\|\xe2\x9d\x8c" "$OUTPUT_FILE"; then
