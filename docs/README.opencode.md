...
```
Clone https://github.com/aaione/omnipowers to ~/.config/opencode/omnipowers, then create directory ~/.config/opencode/plugins, then symlink ~/.config/opencode/omnipowers/.opencode/plugins/omnipowers.js to ~/.config/opencode/plugins/omnipowers.js, then symlink ~/.config/opencode/omnipowers/skills to ~/.config/opencode/skills/omnipowers, then restart opencode.
```
...
```bash
ls -l ~/.config/opencode/plugins/omnipowers.js
ls -l ~/.config/opencode/skills/omnipowers
```

Both should show symlinks pointing to the omnipowers directory.
...
git clone https://github.com/aaione/omnipowers.git "%USERPROFILE%\.config\opencode\omnipowers"
...
del "%USERPROFILE%\.config\opencode\plugins\omnipowers.js" 2>nul
rmdir "%USERPROFILE%\.config\opencode\skills\omnipowers" 2>nul

:: 4. Create plugin symlink (requires Developer Mode or Admin)
mklink "%USERPROFILE%\.config\opencode\plugins\omnipowers.js" "%USERPROFILE%\.config\opencode\omnipowers\.opencode\plugins\omnipowers.js"

:: 5. Create skills junction (works without special privileges)
mklink /J "%USERPROFILE%\.config\opencode\skills\omnipowers" "%USERPROFILE%\.config\opencode\omnipowers\skills"
...
# 3. Remove existing links (safe for reinstalls)
rm -f ~/.config/opencode/plugins/omnipowers.js 2>/dev/null
rm -rf ~/.config/opencode/skills/omnipowers 2>/dev/null

# 4. Create plugin symlink (requires Developer Mode or Admin)
cmd //c "mklink \"$(cygpath -w ~/.config/opencode/plugins/omnipowers.js)\" \"$(cygpath -w ~/.config/opencode/omnipowers/.opencode/plugins/omnipowers.js)\""

# 5. Create skills junction (works without special privileges)
cmd //c "mklink /J \"$(cygpath -w ~/.config/opencode/skills/omnipowers)\" \"$(cygpath -w ~/.config/opencode/omnipowers/skills)\""
...
3. **Omnipowers skills** (`~/.config/opencode/skills/omnipowers/`) - via symlink

## Features

### Automatic Context Injection

The plugin automatically injects omnipowers context via the `experimental.chat.system.transform` hook. This adds the "using-omnipowers" skill content to the system prompt on every request.

### Native Skills Integration

Omnipowers uses OpenCode's native `skill` tool for skill discovery and loading. Skills are symlinked into `~/.config/opencode/skills/omnipowers/` so they appear alongside your personal and project skills.
...
**Location:** `~/.config/opencode/omnipowers/.opencode/plugins/omnipowers.js`

**Components:**
- `experimental.chat.system.transform` hook for bootstrap injection
- Reads and injects the "using-omnipowers" skill content

### Skills

**Location:** `~/.config/opencode/skills/omnipowers/` (symlink to `~/.config/opencode/omnipowers/skills/`)
...
1. Check plugin exists: `ls ~/.config/opencode/omnipowers/.opencode/plugins/omnipowers.js`
...
1. Verify skills symlink: `ls -l ~/.config/opencode/skills/omnipowers` (should point to omnipowers/skills/)
...
1. Verify using-omnipowers skill exists: `ls ~/.config/opencode/omnipowers/skills/using-omnipowers/SKILL.md`
