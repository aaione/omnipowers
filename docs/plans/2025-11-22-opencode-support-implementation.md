...
> **For Claude:** REQUIRED SUB-SKILL: Use omnipowers:executing-plans to implement this plan task-by-task.

**Goal:** Add full omnipowers support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/omnipowers-codex` (lines 40-74)
...
- Reference: `.codex/omnipowers-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'omnipowers' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/omnipowers-codex` (lines 212-280)
 ...
 * (personal skills override omnipowers skills).
 *
 * @param {string} skillName - Name like "omnipowers:brainstorming" or "my-skill"
 * @param {string} omnipowersDir - Path to omnipowers skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, omnipowersDir, personalDir) {
    // Strip omnipowers: prefix if present
    const forceOmnipowers = skillName.startsWith('omnipowers:');
    const actualSkillName = forceOmnipowers ? skillName.replace(/^omnipowers:/, '') : skillName;

    // Try personal skills first (unless explicitly omnipowers:)
    if (!forceOmnipowers && personalDir) {
    ...
    // Try omnipowers skills
    if (omnipowersDir) {
        const omnipowersPath = path.join(omnipowersDir, actualSkillName);
        const omnipowersSkillFile = path.join(omnipowersPath, 'SKILL.md');
        if (fs.existsSync(omnipowersSkillFile)) {
            return {
                skillFile: omnipowersSkillFile,
                sourceType: 'omnipowers',
                skillPath: actualSkillName
            };
        }
    }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/omnipowers-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/omnipowers-codex` (add import at top)
...
Run: `node -c .codex/omnipowers-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/omnipowers-codex
...
**Files:**
- Modify: `.codex/omnipowers-codex` (lines 40-74)
...
Run: `.codex/omnipowers-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/omnipowers-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/omnipowers-codex` (lines 16-38, approximately)
...
Run: `.codex/omnipowers-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/omnipowers.js`
...
/**
 * Omnipowers plugin for OpenCode.ai
 *
...
const omnipowersSkillsDir = path.join(homeDir, '.config/opencode/omnipowers/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const OmnipowersPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/omnipowers.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/omnipowers.js
...
**Files:**
- Modify: `.opencode/plugin/omnipowers.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "omnipowers:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > omnipowers)
...
Run: `node -c .opencode/plugin/omnipowers.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the omnipowers and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const omnipowersSkills = skillsCore.findSkillsInDir(
            omnipowersSkillsDir,
            'omnipowers',
            3
          );
          ...
          const allSkills = [...personalSkills, ...omnipowersSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install omnipowers skills to ~/.config/opencode/omnipowers/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'omnipowers:';
...
```javascript
    'session.started': async () => {
      // Read using-omnipowers skill content
      const usingOmnipowersPath = skillsCore.resolveSkillPath(
        'using-omnipowers',
        omnipowersSkillsDir,
        personalSkillsDir
      );

      let usingOmnipowersContent = '';
      if (usingOmnipowersPath) {
        const fullContent = fs.readFileSync(usingOmnipowersPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingOmnipowersContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Omnipowers skills: \`omnipowers:skill-name\` (from ~/.config/opencode/omnipowers/skills/)
- Personal skills: \`skill-name\` (from ~/.config/opencode/skills/)
- Personal skills override omnipowers skills when names match
`;

      // Check for updates (non-blocking)
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/omnipowers')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/omnipowers && git pull` to update omnipowers.' :
        '';
...
**Below is the full content of your 'omnipowers:using-omnipowers' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingOmnipowersContent}
...
### 1. Install Omnipowers Skills

```bash
# Clone omnipowers skills to OpenCode config directory
mkdir -p ~/.config/opencode/omnipowers
git clone https://github.com/aaione/omnipowers.git ~/.config/opencode/omnipowers
```

### 2. Install the Plugin

The plugin is included in the omnipowers repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/omnipowers/.opencode/plugin/omnipowers.js`
...
ln -s ~/.config/opencode/omnipowers/.opencode/plugin/omnipowers.js .opencode/plugin/omnipowers.js
```
...
```
use use_skill tool with skill_name: "omnipowers:brainstorming"
```
...
Personal skills override omnipowers skills with the same name.
...
1. Check plugin file exists: `ls ~/.config/opencode/omnipowers/.opencode/plugin/omnipowers.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/omnipowers/skills`
...
- Documentation: https://github.com/aaione/omnipowers
```
...
Omnipowers works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/omnipowers-codex use-skill omnipowers:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/omnipowers-codex bootstrap | head -30`
...
ls -l .opencode/plugin/omnipowers.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/omnipowers.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/omnipowers-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/omnipowers-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/omnipowers.js` created with tools and hooks
