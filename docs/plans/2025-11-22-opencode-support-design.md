...
Add full omnipowers support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port omnipowers to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/omnipowers-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/omnipowers.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/omnipowers/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/omnipowers-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "omnipowers:brainstorming")')
  }),
...
1. **Inject using-omnipowers content**
   - Full content of the using-omnipowers skill
...
```javascript
// .opencode/plugin/omnipowers.js
const skillsCore = require('../../lib/skills-core');
const path = require('path');
const fs = require('fs');
const { z } = require('zod');

export const OmnipowersPlugin = async ({ client, directory, $ }) => {
  const omnipowersDir = path.join(process.env.HOME, '.config/opencode/omnipowers');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingOmnipowers = await readSkill('using-omnipowers');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingOmnipowers}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── omnipowers-codex        # UPDATED: Use skills-core
│   ├── omnipowers-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── omnipowers.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/omnipowers-codex`
...
2. Update `.codex/omnipowers-codex` to use shared core
...
1. Create `.opencode/plugin/omnipowers.js`
